...
void init_fs() {
  ...
  Log("the width is %d, height is %d", _screen.width, _screen.height);
...
//	Log("This is Lipin");
	for(int i=0; i<NR_FILES; i++)
	{
		...
		{
//			Log("i is %d, NR_FILES is %d", i, NR_FILES);
			return i;
		}
	}
	assert(0);
	...
	switch(fd){
		...
		default:
			if (file_table[fd].open_offset == f_size)
				return 0;
			else if (file_table[fd].open_offset + len > f_size)
				len = f_size - file_table[fd].open_offset;
			ramdisk_read(buf, file_table[fd].disk_offset+file_table[fd].open_offset, len);
			file_table[fd].open_offset+=len;
			break;
	}
//    Log("this is fs log1 %d", len);
	return len;
}
