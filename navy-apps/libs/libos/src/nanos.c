...
//	_exit(SYS_open);
//  _syscall_(SYS_exit, -SYS_open, 0, 0);
    uintptr_t path_t=path;
	_syscall_(SYS_open, path_t, flags, mode);
}
...
extern char _end;
intptr_t p_brk = (intptr_t) &_end;

void *_sbrk(intptr_t increment){
  intptr_t p_brk_old=p_brk;
  _syscall_(SYS_brk, p_brk_old+increment, 0, 0);
  p_brk=p_brk+increment;
  return (void *)p_brk_old;
}
