...
static inline make_DopHelper(SI) {
  ...
  if(op->width==1)
  {
	  uint8_t temp = instr_fetch(eip, op->width);
	  int8_t temp_s=temp;
	  int32_t res=temp_s;
	  op->simm=res;
  }
...
static inline make_DopHelper(SI_24) {
  ...
  {
    uint16_t temp = instr_fetch(eip, op->width);
    int16_t temp_s=temp;
    int32_t res=temp_s;
    op->simm=res;
...
make_DHelper(movzbl_E2G) {
  id_src->width=1;
  decode_op_rm(eip, id_src, true, id_dest, false);
}

make_DHelper(movswl_Ew2G) {
  id_src->width=2;
  decode_op_rm(eip, id_src, true, id_dest, false);
}

make_DHelper(movswl_Eb2G) {
  id_src->width=1;
  decode_op_rm(eip, id_src, true, id_dest, false);
}

make_DHelper(movzwl_Ew2G) {
  id_src->width=2;
  decode_op_rm(eip, id_src, true, id_dest, false);
}
...
make_DHelper(xor_r2E) {
  decode_op_rm(eip, id_dest, true, id_src, true);
//Log("id_dest->val is %x, id_src->val is %x",id_dest->val,id_src->val);
}
