...
//  TODO();
  if(cpu.eax==r.eax&&cpu.ebx==r.ebx&&cpu.ecx==r.ecx&&cpu.edx==r.edx&&cpu.esp==r.esp&&cpu.ebp==r.ebp&&cpu.esi==r.esi&&cpu.edi==r.edi&&cpu.eip==r.eip)
	diff=false;
  else
	  diff=true;
  if (diff) {
	Log("r.eax is %x, r.ecx is %x, r.edx is %x, r.ebx is %x, r.esp is %x, r.ebp is %x, r.esi is %x, r.edi is %x, r.eip is %x, ", r.eax, r.ecx, r.edx, r.ebx, r.esp, r.ebp, r.esi, r.edi, r.eip);
    nemu_state = NEMU_END;
  }
}
